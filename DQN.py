...
class Network():
    ...
    def _createModel(self):
        ...
        x1 = slim.fully_connected(self.input, 8, scope='fc/fc_1')
        x1 = tf.nn.relu(x1)
...
class Agent():
    ...
    def replay(self, sess, logWriter):
        ...
        probs = self.network.predict(sess, states)
        probs_ = self.network.predict(sess, states_)

        inputStates = np.zeros((batchSize, self.stateSize))
        inputTdTargets = np.zeros((batchSize, self.actionsSize))
        ...
        for i in range(batchSize):
            ...
            if s_ is not None:
                ...
                tdTarget[a] = r + self.td_discount_rate * np.amax(probs_[i])
